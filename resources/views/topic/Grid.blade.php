    ...
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">موضوعات
                <span class="text-info">( {{$subject->title}} )</span>
            </h3>
            <div class="card-tools">
                <button type="button" class="btn btn-app" data-toggle="modal" data-target="#searchModal">
                    <i class="fa fa-search text-info"></i>جستجو
                </button>
                <a class="btn btn-app" href="{{route('topic.create', ['subjectId'=>$subject->id])}}">
                    <i class="fa fa-plus text-success"></i>ثبت
                </a>
                <a class="btn btn-app" href="{{url('subject')}}">
                    <i class="fa fa-arrow-left text-warning "></i>بازگشت
                </a>
            </div>
        </div>
        <div class="card-body">
                        ...
                        <tr>
                            ...
                            <td align="left">
                                ...
                                <div class="dropdown-menu">
                                    ...
                                    <form method="POST" action="{{route('topic.destroy', ["id"=>$item->id, "subject"=>$subject->id])}}">
                                        {{ csrf_field() }}
                                        {{ method_field('DELETE') }}
