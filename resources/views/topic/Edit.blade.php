...
@section('content')
    ...
    <div class="card">
        <div class="card-header mb-5">
            <h3 class="card-title">موضوع جدید</h3>
        ...
        <div class="card-body">
            @if($entity->id)
                <form action="{{route('topic.update', $entity->id)}}" method="POST">
                    @method('PATCH')
                    @else
                        <form method="POST" action="{{ route('topic.store') }}">
                            @endif
                            <input id="subjectId" name="subject_id" type="hidden" class="form-control" value="{{$subjectId}}">
                            @csrf
                            <div class="row">
