function initCalendarUtility() {
    ...
    $('[class*=persianCalender]').on("keydown", function (e) {
        var keycode;
        if (window.event)
...
function setCalenderClass() {
    $(document).find("[class*=persianCalender][type=text]").each(function (index, element) {
        ...
        if (!getRules) {
            $(this).datepicker({dateFormat: 'yy/mm/dd', showButtonPanel: true, changeMonth: true, changeYear: true, duration: "slow", yearRange: "1330:1440", showAnim: "show"});
            return;
        }
        var str = getRules[1];
        var rules = str.split(/\[|,|\]/);
